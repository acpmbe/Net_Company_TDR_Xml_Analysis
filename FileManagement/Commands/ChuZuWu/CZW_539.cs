...
namespace FileManagement.Commands.ChuZuWu
{
    public class CZW_539 : ICommand
    {
        ...
        public CZW_539(string _cmdId, byte[] _content, ulong _StationId, DateTime _PlatformTime)
        {
            this.CmdId = _cmdId;
        ...
        private string Name { get { return "上传房东授权"; } }
        public bool Execute()
        {
        ...
        private Pro_IndateBase_MJNew_Mod GetInfo_New()
        {
            ...
            {
                CardType = "0";
                ICCard = ConverUtil.ByteToStr_Q(_content, 10, 8);  //房东身份证。         
            ...
            info.pi_policecard = ConverUtil.ByteToStr_4(_content, 18);  //警察卡。
        ...
        private Pro_IndateBase_MJNew_Mod GetInfo_Old()
        {
            ...
            {
                CardType = "0";
                ICCard = ConverUtil.ByteToStr_Q(_content, 4, 8);  //房东身份证。         
                HouseNum = IsIdentitycard.ToString();
            }    
            info.pi_bigtype = "2";
            info.pi_protocoltype = CmdId;
            info.pi_devicecode = ConverUtil.ByteToStr_4(_content, 0);  //门戒编号。
            info.pi_devicetime = PlatformTime;
            info.pi_cardtype = CardType;
            info.pi_cardid = ICCard;
            info.pi_houseno = HouseNum;
            info.pi_policecard = ConverUtil.ByteToStr_4(_content, 12);  //警察卡。
            info.pi_stationno = StationId.ToString();
            info.pi_servicetime = PlatformTime;
