...
{
 public   class Pro_ShipDevice_Station_Dal
    {



        public static void Exec(Pro_ShipDevice_Station_Mod info, out UInt16 resultNum, out string reason)
        {

            Database db = DataConnect.GetConnect;
            DbCommand cmd = db.GetStoredProcCommand("PRO_SHIPDEVICE_STATION");
            db.AddInParameter(cmd, ":pi_devid", DbType.String, info.Pi_DevId);
            db.AddInParameter(cmd, ":pi_devtime", DbType.Date, info.Pi_DevTime);
            db.AddInParameter(cmd, ":pi_station", DbType.String, info.Pi_Station);
